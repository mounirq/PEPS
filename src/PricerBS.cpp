...
void PricerBS::price(double &prix, double &ic)
{
    ...
    prix = pnl_bs_call(spot, ((VanillaOption*)opt_)->strike_, opt_->T_, mod_->r_, 0, sigma);
}
...
void PricerBS::price(const PnlMat *past, double t, double &prix, double &ic)
{
    ...
    double K = ((VanillaOption*)opt_)->strike_;
    double r = mod_->r_;
    double sigma = pnl_vect_get(mod_->sigma_, 0);
    double T = opt_->T_;
...
//    prix = S0 * pnl_cdfnor(d1) - K * exp(-r*(T-t)) * pnl_cdfnor(d2);
    ...
    prix = pnl_bs_call(S0, ((VanillaOption*)opt_)->strike_, opt_->T_-t, mod_->r_, 0, sigma);
}
...
void PricerBS::delta(const PnlMat *past, double t, PnlVect *delta, PnlVect* icDelta)
{
    ...
    pnl_cf_call_bs(S0, ((VanillaOption*)opt_)->strike_, opt_->T_-t, mod_->r_, 0, sigma, &prix, &delta_double);
