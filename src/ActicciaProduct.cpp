...
ActicciaProduct::ActicciaProduct(double T, int nbTimeSteps, int size):Option(T,nbTimeSteps,size){
    vlr_ = 100;
}
...
double ActicciaProduct::payoff(const PnlMat *path) {
	...
	for (int i = 0; i< size_; i++){
		...
		initialValue = GET(vector, 0);
		finalValue = GET(vector, nbTimeSteps_);
		tmp = (finalValue - initialValue)/initialValue;
